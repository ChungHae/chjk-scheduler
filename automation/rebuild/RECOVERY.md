...
- r151: live a9c50d36 / test 70ae1f35 (2026-08-26 작업본, PC test 파일 = r151, 미배포 — 매입매출 전체 속도 개선. 표시 수치·판정 로직은 완전히 동일(1,653행 전 항목 대조 일치), 계산 시점과 탐색 방식만 변경. (A) 원장 캐시: _fxDataStamp(=_fxCacheBump + 각 배열 길이)가 같으면 _fxLedgers/_fxDupCandidates 결과 재사용. _fxCacheBump 는 _fxSave()/_fxSaveBig() 첫 줄, _fxEnsureData 로드 완료, 외부 동기화 재적재(load 블록) 4곳에서 ++ → 길이가 안 변하는 제자리 수정(입금 배정, 결제조건 변경, 조정 금액 수정)도 반드시 재계산됨(검증 완료). (B) _fxDupCandidates O(거래처²)→O(거래처): 사업장+정규화이름으로 묶어 같은 묶음 안에서만 비교(짝 순서 i<j 동일). (C) _fxLedgersOne 의 기초이월 매칭·N|이름 병합을 이름 인덱스(Object.create(null))로 O(n)화. 거래처명이 __proto__/constructor 여도 안전(검증 완료). (D) _fxDue 를 _fxDueCalc + 메모이제이션으로 분리(8,008 케이스 원본과 완전 일치). (E) 미수현황·거래처별집계 검색창 180ms 디바운스. (F) fxArXls 가 캐시 배열을 in-place sort 하지 않도록 slice() 추가. 실측(거래처 1,740곳/매출 15,660건/매입 12,528건/입금 10,840건): 첫 진입 365→28ms, 페이지 이동 316→27ms, 상태필터 350→3ms, 검색 1회 344→2ms, 사업장 전환 157→12ms, 5글자 연속 타이핑 멈춤 1,829→0ms)
