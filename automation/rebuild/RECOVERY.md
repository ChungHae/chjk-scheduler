...
- r149: live 905803bf / test 0490fd2d (2026-08-25 작업본, PC test 파일 = r149, 미배포 — 업체 목록이 개수조차 안 뜨고 조용히 멈추는 버그 수정: clientList 배열에 null 등 손상된 항목이 하나라도 섞이면 _clxRender()의 sort 비교자에서 TypeError가 발생해 함수 전체가 그 자리에서 멈춤(Firebase 배열 동기화 시 index 삭제로 null이 채워질 수 있음). allClients()에서 손상된 항목을 걸러내도록 방어 처리 — 이 함수는 앱 전역에서 공통으로 쓰여 다른 화면의 동일 증상도 함께 예방됨. 재현 테스트로 원인 확정(수정 전 정확히 이 TypeError 재현, 수정 후 정상 렌더링 확인))
