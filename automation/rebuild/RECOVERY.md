...
- r146: live c4174099 / test 704159f6 (2026-08-25 작업본, PC test 파일 = r146, 미배포 — 거래처 중복 원장 버그 수정: 원장 병합(N|이름→사업자번호 슬롯, 기초이월)의 이름 비교가 완전일치라 공백차이만 있어도 병합 실패 → 잔액 0인데 채권연령 안바뀌는 원인. 공백 정규화로 자동병합 완화 + 관리자용 '중복 거래처 후보' 진단 패널(법인표기·대소문자 무시) 및 확인 후 병합 버튼(계산서·입금·조정·기초이월·결제조건 전부 이관) 추가)
