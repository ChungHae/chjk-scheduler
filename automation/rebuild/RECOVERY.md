...
- r147: live b68f46f7 / test 5c47890a (2026-08-25 작업본, PC test 파일 = r147, 미배포 — 채권 연령 계산 버그 수정: 마이너스(수정) 세금계산서·음수 기초이월이 채무(obls) FIFO에 그대로 섞여, 전체 잔액은 0인데 앞쪽에서 이미 "미수"로 확정된 잔여물이 연령 구간(예 90일 초과)에 그대로 남는 문제. 계산서·기초이월도 조정과 동일하게 "양수만 채무, 음수는 변제(credit)"로 통일 — 잔액 0이면 연령도 항상 함께 0이 되도록 수학적으로 보장)
